...
const KEYWORD_MAP = {
  ...
  leak: ["plumbing", "concrete_repair"],
  pooling: ["plumbing", "concrete_repair", "pressure_washing"],
  flood: ["plumbing", "mold_remediation"],
  drain: ["plumbing"],
  pipe: ["plumbing"],
  faucet: ["plumbing", "handyman"],
  toilet: ["plumbing", "janitorial"],
  floor: ["floor_care", "carpet_cleaning", "concrete_repair"],
  carpet: ["carpet_cleaning"],
  stain: ["carpet_cleaning", "pressure_washing", "janitorial"],
  scuff: ["floor_care"],
  tile: ["floor_care", "handyman"],
  window: ["window_washing"],
  glass: ["window_washing", "handyman"],
  dirty: ["janitorial", "pressure_washing"],
  trash: ["waste_management", "janitorial"],
  garbage: ["waste_management"],
  smell: ["janitorial", "pest_control", "mold_remediation"],
  odor: ["janitorial", "mold_remediation"],
  mold: ["mold_remediation"],
  mildew: ["mold_remediation"],
  bug: ["pest_control"],
  pest: ["pest_control"],
  mouse: ["pest_control"],
  rat: ["pest_control"],
  ...
  bulb: ["lighting"],
  dark: ["lighting", "electrical"],
  flicker: ["lighting", "electrical"],
  outlet: ["electrical"],
  power: ["electrical"],
  wire: ["electrical"],
  hot: ["hvac"],
  ...
  thermostat: ["hvac"],
  paint: ["painting"],
  wall: ["painting", "handyman"],
  crack: ["concrete_repair", "painting", "handyman"],
  hole: ["handyman", "concrete_repair"],
  door: ["handyman"],
  hinge: ["handyman"],
  broken: ["handyman", "plumbing", "electrical"],
  fix: ["handyman"],
  parking: ["parking_lot"],
  pothole: ["parking_lot", "concrete_repair"],
  asphalt: ["parking_lot"],
  lot: ["parking_lot"],
  stripe: ["parking_lot"],
  grass: ["landscaping"],
  ...
  salt: ["snow_removal"],
  sign: ["signage"],
  camera: ["security_systems"],
  alarm: ["security_systems"],
  lock: ["security_systems", "handyman"],
  kitchen: ["kitchen_cleaning"],
  grease: ["kitchen_cleaning", "pressure_washing"],
  hood: ["kitchen_cleaning"],
  sanitize: ["disinfection"],
  disinfect: ["disinfection"],
  covid: ["disinfection"],
  pressure: ["pressure_washing"],
  wash: ["pressure_washing", "window_washing"],
  exterior: ["pressure_washing", "painting", "window_washing"],
  sidewalk: ["pressure_washing", "concrete_repair", "snow_removal"],
  concrete: ["concrete_repair"],
  ceiling: ["handyman", "painting"],
  roof: ["handyman"],
};
  ...
  Object.entries(KEYWORD_MAP).forEach(([keyword, serviceIds]) => {
    if (lower.includes(keyword)) {
      serviceIds.forEach((id, i) => {
        scores[id] = (scores[id] || 0) + (3 - i);
      });
    }
  });

  const sorted = Object.entries(scores).sort((a, b) => b[1] - a[1]);
  if (sorted.length === 0) return [SERVICE_CATEGORIES[8]]; // default handyman
...
function getPriority(text) {
  const lower = text.toLowerCase();
  if (lower.match(/urgent|emergency|flood|dangerous|hazard|safety|immediate|asap/)) return "Critical";
  if (lower.match(/leak|broken|damage|crack|not working|failed|malfunction/)) return "High";
  if (lower.match(/wear|dirty|stain|smell|scheduled|routine/)) return "Medium";
  return "Standard";
}
...
export default function FacilityServiceAgent() {
  ...
  const [phase, setPhase] = useState("idle"); // idle, classifying, matching, drafting, bundling, done
  ...
  const EXAMPLE_REQUESTS = [
    ...
    "Kitchen exhaust hood hasn't been cleaned in months, grease buildup is visible",
  ];

  async function runAgent(text) {
    setResult(null);
    setPhase("classifying");
    setPhaseText("Analyzing service request with NLP...");

    await delay(1200);

    const classified = classifyRequest(text);
    const serviceIds = classified.map((c) => c.id);
    const priority = getPriority(text);
    const timeline = getTimeline(priority);

    setPhase("matching");
    setPhaseText("Querying Dataverse for contractor availability...");
    await delay(1000);

    const contractors = matchContractors(serviceIds);

    setPhase("drafting");
    setPhaseText("Generating work order from classification...");
    await delay(900);

    const upcoming = UPCOMING_SERVICES[facility.id] || [];
    const bundleOpps = upcoming.filter((u) => {
      const daysDiff = Math.random() * 10;
      return daysDiff < 8;
    });

    setPhase("bundling");
    setPhaseText("Checking for bundling opportunities...");
    await delay(800);
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        textarea:focus { outline: none; border-color: var(--accent) !important; box-shadow: 0 0 0 3px var(--accent-glow); }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
            ...
            <div>
              ...
              <p style={{
                ...
                City Wide Facility Solutions — Dynamics 365 + Agentic AI Demo
              </p>
            </div>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 32px" }}>
        ...
        <div className="fade-up" style={{ marginBottom: 24 }}>
          <label style={{
            ...
            letterSpacing: "0.08em", color: "var(--text-secondary)", display: "block", marginBottom: 8,
          }}>Active Facility</label>
          <select
            value={facility.id}
            onChange={(e) => setFacility(FACILITIES.find((f) => f.id === +e.target.value))}
            style={{
              fontFamily: "'DM Sans', sans-serif",
          ...
          >
            {FACILITIES.map((f) => (
              <option key={f.id} value={f.id}>
                {f.name} — {f.type} ({f.sqft} sq ft)
              </option>
            ))}
          </select>
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "var(--text-secondary)", marginTop: 6 }}>
            📍 {facility.address}
          </p>
        </div>
        ...
        <div className="fade-up stagger-1" style={{ marginBottom: 20 }}>
          ...
          <textarea
            ...
            style={{
              ...
              padding: "14px 16px", fontSize: 14, width: "100%",
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12 }}>
            <button
              ...
              style={{
                fontFamily: "'DM Sans', sans-serif",
                background: isProcessing ? "#1e293b" : "linear-gradient(135deg, #3b82f6, #6366f1)",
        ...
        <div className="fade-up stagger-2" style={{ marginBottom: 32 }}>
          <p style={{
            fontFamily: "'DM Sans', sans-serif",
            fontSize: 11, color: "var(--text-secondary)", marginBottom: 8, fontWeight: 500,
          }}>Try an example:</p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {EXAMPLE_REQUESTS.map((ex, i) => (
              <div
                ...
                onClick={() => { setInput(ex); setResult(null); setPhase("idle"); }}
                style={{
                  fontFamily: "'DM Sans', sans-serif",
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 16 }}>
              {["classifying", "matching", "drafting", "bundling"].map((p, i) => (
                <div key={p} style={{
                  ...
                  background: ["classifying", "matching", "drafting", "bundling"].indexOf(phase) >= i
          ...
          <div ref={resultRef}>
            ...
            <div className="fade-up stagger-1" style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, color: "var(--text-primary)" }}>
                  Service Classification
                </h3>
                <span style={{
                  fontFamily: "'JetBrains Mono', monospace",
                  fontSize: 11, color: "var(--green)",
                  background: "var(--green-glow)", padding: "3px 10px", borderRadius: 4,
                }}>
                  AI Classified
                </span>
              </div>
                  ...
                  <div key={svc.id} style={{
                    ...
                    <div>
                      ...
                      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "var(--text-secondary)" }}>
                        {i === 0 ? "Primary" : `Alt ${i}`} • Confidence {Math.round(95 - i * 12)}%
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="fade-up stagger-2" style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600 }}>
                  Draft Work Order
                </h3>
                <span style={{
                  fontFamily: "'JetBrains Mono', monospace",
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{
                  ...
                  background: "#0a0f1a", borderRadius: 6, padding: "10px 14px",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {result.contractors.map((c, i) => (
                  <div key={c.id} className="contractor-card" style={{
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        ...
                        {i === 0 && (
                          <span style={{
                            ...
                            background: "var(--green-glow)", padding: "2px 8px", borderRadius: 4,
                          }}>Best Match</span>
                        )}
                        {!c.available && (
                          <span style={{
                    ...
                    <button style={{
                      fontFamily: "'DM Sans', sans-serif",
                      background: c.available ? "var(--accent)" : "var(--border)",
            ...
            {result.bundleOpps.length > 0 && (
              <div className="fade-up stagger-4" style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  <h3 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, color: "#a78bfa" }}>
                ...
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "var(--text-secondary)", marginBottom: 12, lineHeight: 1.6 }}>
                  This facility has upcoming services that could be combined with this work order to reduce visit costs:
                </p>
            ...
            <div className="fade-up stagger-5" style={{
              ...
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: "var(--text-secondary)", marginBottom: 16, lineHeight: 1.5 }}>
                This request touched the following Dataverse tables in the classification pipeline:
              </p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {[
                  "cw_facility", "cw_servicerequest", "cw_servicecategory",
                  "cw_contractor", "cw_workorder", "cw_scheduledservice",
                ].map((entity) => (
